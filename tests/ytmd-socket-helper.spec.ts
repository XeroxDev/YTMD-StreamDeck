...
describe('YTMD-Socket-Helper Socket', () => {
    ...
    beforeEach(() => {
        ...
        server.on('connection', (socket) => {
            ...
            socket.on('retrieve-info', () => socket.emit('info', true));

            socket.on('query-player', () => socket.emit('player', true));

            socket.on('query-track', () => socket.emit('track', true));

            socket.on('query-queue', () => socket.emit('queue', true));

            socket.on('query-playlist', () => socket.emit('playlist', true));

            socket.on('query-lyrics', () => socket.emit('lyrics', true));
        });
    });
    ...
    describe('connection', () => {
        ...
        it('should connect', (done) => {
            ...
            const sub2 = client.onError$.subscribe(() => {
                test++;
                sub2.unsubscribe();
                if (test === 0) done(Error('Error while connecting'));
            });
            client.setConnection({
                port: '5000',
                host: 'localhost',
